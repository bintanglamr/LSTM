...
Original file is located at
    https://colab.research.google.com/drive/1YjDj-wyvcPVWYaOlHKeibQRBnT1XsJO3
...
# Model Training, Prediction, and Evaluation for each horizon using LSTM
results_lstm = {}
predictions_lstm = {}
histories_lstm = {}  # Store the training history for each horizon
...
for h in horizons:
    ...
    lstm_model = tf.keras.Sequential([
        tf.keras.layers.LSTM(units=100, activation='relu', input_shape=(X_train.shape[1], X_train.shape[2])),
    ...
    history = lstm_model.fit(X_train, y_train, epochs=100, batch_size=64, validation_split=0.15, shuffle=False)
    histories_lstm[h] = history  # Save the training history
    ...
    predictions_lstm[h] = (y_test_inv.flatten(), y_pred_inv.flatten())

    # Evaluation
    def LSTM_accuracy_metrics(y_true, y_pred):
    ...
    metrics = LSTM_accuracy_metrics(y_test_inv, y_pred_inv)
    results_lstm[h] = metrics

# Plot loss for each horizon using LSTM
plt.figure(figsize=(15, 5))
for i, h in enumerate(horizons):
    plt.subplot(1, len(horizons), i + 1)
    plt.plot(histories_lstm[h].history['loss'], label='Training Loss')
    plt.plot(histories_lstm[h].history['val_loss'], label='Validation Loss')
...
plt.suptitle('LSTM Model Loss Across Horizons', fontsize=16)
...
fig.suptitle('LSTM GHI Forecasting Comparison', fontsize=17)

for i, h in enumerate(horizons):
    axs[i].plot(predictions_lstm[h][0], marker='.', label='True')
    axs[i].plot(predictions_lstm[h][1], 'r', label='Predicted')
...
for h in horizons:
    print(f"\n{h}-Hour Ahead:")
    for metric, value in results_lstm[h].items():
...
for h in horizons:
    metrics_list.append({
        'Horizon (Hours)': h,
        'R^2': results_lstm[h]['R^2'],
        'MAE': results_lstm[h]['MAE'],
        'MSE': results_lstm[h]['MSE'],
        'RMSE': results_lstm[h]['RMSE']
    })
...
horizons = list(results_lstm.keys())
rmse_values = [results_lstm[h]['RMSE'] for h in horizons]

# Create a bar graph
plt.figure(figsize=(10, 6))
bars = plt.bar(horizons, rmse_values, color='skyblue')
plt.title('LSTM RMSE Comparison Across Forecasting Horizons')
...
# Add a master title for the entire figure
fig.suptitle('LSTM Regression Plot', fontsize=17)
...
for i, h in enumerate(horizons):
    y_true = predictions_lstm[h][0]  # Actual GHI values
    y_pred = predictions_lstm[h][1]   # Predicted GHI values
    r2_value = results_lstm[h]['R^2'] # R² value
...
# Add the Date column
results_df['Date'] = test.index[:len(predictions_lstm[1][0])]
...
results_df['Actual GHI'] = actual_ghi_values[:len(predictions_lstm[1][0])]

# Add predictions for each horizon
for h in horizons:
    pred_col_name = f'Predicted GHI ({h}-Hour Ahead)'
    pred_values = predictions_lstm[h][1]  # Get prediction values
...
# Save to CSV
results_df.to_csv('LSTM07_ghi_predictions.csv', index=False)

print("CSV file 'LSTM07a_ghi_predictions.csv' has been created successfully.")

# Save to Excel
results_df.to_excel('GRU07a_ghi_predictions.xlsx', index=False)
